...
async def _aserialize_files(obj: Any) -> Any:
    ...
    if (
        hasattr(obj, "aget_presigned_url")
        and hasattr(obj, "mimetype")
        and hasattr(obj, "filename")
    ):
        url = await obj.aget_presigned_url()
        return {
            "filename": obj.filename,
            "mimetype": obj.mimetype,
            "url": url,
...
def awebhook_dispatch(
    event_prefix: str = "",
) -> Callable[[Callable[..., Any]], Callable[..., Any]]:
    """Decorator to handle webhook lifecycle events (STARTED, COMPLETED, FAILED) asynchronously."""
...
__all__ = ["dispatch_webhook", "webhook_dispatch", "adispatch_webhook", "awebhook_dispatch"]
